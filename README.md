...
- [Features](#features)
- [TPC-H Dataset generation](#tpc-h-dataset-generation)
- [PostgreSQL setup](#postgresql-setup)
- [PostgreSQL TPC-H Go Client setup](#postgresql-tpc-h-go-client-setup)
- [Executing the client](#executing-the-client-with-docker)
...
Then, download PostgreSQL v16.2's source code [here](https://www.postgresql.org/ftp/source/v16.2/) and pg_ivm 1.8 [here](https://github.com/sraoss/pg_ivm/releases). Extract them into folders named, respectively, `Postgresql/postgresql-16.2` and `pg_ivm-1.8`. Afterwards, download the scripts in the [scriptsPostgreSQL](./scriptsPostgreSQL) folder of this repository.
...
Note for VLDB reviewers: use `64G` as `$ramdisk_size` in order to more accurately replicate our test environment.
...
**Note for VLDB reviewers**: use 1SF for SF and do not change any setting in `postgres_install_ramdisk.sh`, in order to more accurately replicate our test environment.
...
In the [Executing the client with Docker](#executing-the-client-with-docker) section, for all commands replace `andrerj/postgresclient` with `postgresclient`.
...
A full list of all options for configuration files is available on [pgConfigs.go](src/client/pgConfigs.go), method *loadConfigsFile()*.
It is also possible to set most of those settings by using Docker's environment variables: check [pgConfigs.go](src/client/pgConfigs.go), method *loadFlags()* and [start.sh](dockerstuff/start.sh)
...
- `user`: user used to access PostgreSQL. Can be either the user specified in PostgreSQL installation or another user added to PostgreSQL;
...
- `doQueries` and `doUpdates (true/false on both)`: defines if the client should do queries and/or updates. It is recommended to leave these both as `true` and control the update-query rate with `updRate`;
...
This time will, unavoidably, depend on your system and `scale` value: try first running a single client instance only to measure the time it takes for the client and PostgreSQL to be ready. Remember that clients read TPC-H data in order to do updates;
